...
func UpSpaceParseFun(contents []byte, url, dir string) engine.ParseResult {

	var retParseResult engine.ParseResult
	value := gjson.GetManyBytes(contents, "data.list.vlist", "data.page")

	var upid int64
	retParseResult.Requests, upid = getAidDetailReqList(value[0], dir)
	retParseResult.Requests = append(retParseResult.Requests, getNewBilibiliUpSpaceReqList(value[1], upid, dir)...)

	return retParseResult

}

func getAidDetailReqList(pageInfo gjson.Result, dir string) ([]*engine.Request, int64) {
	...
	for _, i := range pageInfo.Array() {
		...
		req := engine.NewRequest(reqUrl, dir, reqParseFunction, fetcher.DefaultFetcher)
...
// 访问up主的时候 需要翻页
func getNewBilibiliUpSpaceReqList(pageInfo gjson.Result, upid int64, dir string) []*engine.Request {
	...
	for i := int64(1); i <= totalPage; i++ {
		...
		req := engine.NewRequest(reqUrl, dir, UpSpaceParseFun, fetcher.DefaultFetcher)
		retRequests = append(retRequests, req)
	}
	return retRequests
}

func GetRequestByUpId(upid int64, dir string) *engine.Request {

	reqUrl := fmt.Sprintf(_getAidUrlTemp, upid, 1)
	return engine.NewRequest(reqUrl, dir, UpSpaceParseFun, fetcher.DefaultFetcher)
}
