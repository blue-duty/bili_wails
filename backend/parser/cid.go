...
func GenGetAidChildrenParseFun(videoAid *model.VideoAid) engine.ParseFunc {
	return func(contents []byte, url, dir string) engine.ParseResult {
		...
		for _, i := range data {
			...
			req := engine.NewRequest(urlApi, dir, GenVideoDownloadParseFun(videoCid), fetcher.DefaultFetcher)
...
func GetRequestByAid(aid int64, dir string) *engine.Request {
